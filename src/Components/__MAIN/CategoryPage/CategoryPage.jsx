...
import {categoryPageScroll, navScroll} from '../../../Utilities/OnScroll/OnScroll'
//STYLE
...
export default class CategoryPage extends PureComponent {
    ...
    getChunkData = async () =>{
        let result = await fetchGet(process.env.REACT_APP_URL_OFFLINE, 'books', null, 'category', this.props.match.params.category)
        console.log(result)
    ...
    render() {
        ...
        return (
            <div 
            id='category-page'
            className={this.props.match.params.category}
            >
                <JumboCategory/>
                <Row>
                    <Col xs={12} sm={8} 
                        ...
                        <div className='page'>
                            {this.state.page.map(book =>{
                                return(
                                    <>
                                        <h3>{book.title}</h3>
                                        <Book cover={book.img} key={book.asin} click={()=>this.selectBooks(book)}/>
                                    </>
                                )
                            })}
                        </div>
